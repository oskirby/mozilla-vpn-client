...
build_deb_source() {
  local release=$1
  local buildtype=$2
  local buildrev=${release}${REVISION}
...
if [ "$SOURCEONLY" == "Y" ]; then
  ...
  for control in ../linux/debian/control.*; do
    ...
    mv mozillavpn_${SHORTVERSION}-*_source.buildinfo $release-$buildtype/ || die "Failed"
    mv mozillavpn_${SHORTVERSION}-*_source.changes $release-$buildtype/ || die "Failed"
    mv mozillavpn_${SHORTVERSION}-*.debian.tar.* $release-$buildtype/ || die "Failed"
    mv mozillavpn_${SHORTVERSION}-*.dsc $release-$buildtype/ || die "Failed"
  done
